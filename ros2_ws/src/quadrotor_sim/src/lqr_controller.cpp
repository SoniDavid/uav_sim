...
{
public:
    LQRControllerNode() : Node("lqr_controller"), state_received_(false), log_cycle_(0)
    {
        ref12_.fill(0.0);
        ...
        timer_ = create_wall_timer(
            ...
            std::bind(&LQRControllerNode::tick, this));

        RCLCPP_INFO(get_logger(), "LQR controller node started at %.0f Hz.", 1.0 / DT_PUB);
    }
...
private:
    void tick()
    {
        ...
        const auto u = compute_lqr_control(state12.data(), ref12.data());
        ...
        if (++log_cycle_ >= 100) {
            ...
            if (T_avg > T_MAX * 0.95) {
                RCLCPP_WARN(get_logger(),
                    "[lqr] Thrust near saturation: avg=%.3f N (T_MAX=%.3f N)", T_avg, T_MAX);
            } else {
                RCLCPP_INFO(get_logger(), "[lqr] Thrust avg=%.3f N", T_avg);
            }
        }
    }
